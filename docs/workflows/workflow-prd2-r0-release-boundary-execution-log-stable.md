...
初始 R0 release-boundary pass 没有执行生产部署、生产数据库 migration、生产写入、登录态操作、provider call、邮件发送、站内通知发送或调度变更。2026-06-21 后续 post-merge release 已在明确授权后执行生产部署和 Alembic migration。2026-06-23 已完成一次明确授权的小范围 M3 GitHub API-first production package gate，并在取证后清理 scoped fixtures；同日已完成 M5 Public Web/RSS/Docs production package smoke，允许一次公开 RSS TaskRun、DatasetVersion save、read-only drift/report preview，并按 exact-ID 与 generic E2E cleanup 清理。后续完成 M5 public-content 多个 scoped gates、retained lifecycle、retained cleanup dry-run、scheduler tick、retained canary scheduler/drift refresh、retained TTL observation baseline/midpoint/final 168h observation 与 deploy marker diagnosis。2026-07-02 在明确授权后，基于 production `42851929d59d82708c9380d36347ca721979297d` 创建 release commit `b81a4be2a47f387d381293db7c4b2932128f6708` 并部署到生产，Alembic 从 `202606110023` 升级到 `202606110026`，生产 active app `HEAD` 与 `.deploy-sha` 均为 `b81a4be2a47f387d381293db7c4b2932128f6708`。Loop 32 已把 release 分支同步到 `origin/codex/release-3b-on-428`，Loop 33 已选择 docs/state sync，Loop 34 已完成 docs/state sync，Loop 35 已验证 release branch 对 `main`/`origin/main` 为 fast-forward-capable，Loop 36 已创建 draft PR #10 `https://github.com/zjgulai/data_achieve/pull/10`。provider call、product/report/subscription email、production retained cleanup execute、生产浏览器运行、浏览器 artifact 写入、scheduler mutation、marker/symlink housekeeping 和 main merge 仍未执行。
...
| R0-27 | Loop 31 production deploy to `b81a4be` | done_deploy | release commit `b81a4be2a47f387d381293db7c4b2932128f6708` fast-forwarded from production `42851929`；local DB-backed MVP gate passed；production deploy preflight passed；Alembic upgraded `202606110023 -> 202606110024 -> 202606110025 -> 202606110026`；`api/web/edge` healthy；public health and authenticated read-only smoke passed |
| R0-28 | Loop 32 post-deploy closeout and source sync | done_read_only_source_sync | production active app `HEAD` and `.deploy-sha` stayed `b81a4be2a47f387d381293db7c4b2932128f6708`; `origin/codex/release-3b-on-428` points to `b81a4be`; no merge, PR, deploy, restart, migration, provider call, email send, cleanup execute, scheduler mutation, production browser run, or marker/symlink housekeeping |
...
| R0-32 | Loop 36 draft PR creation | done_draft_pr | docs-only source-control state sync commit `9e3d2077a3903b627b8f0a489caf8b1acf897616` and later PR-state docs commits pushed to `origin/codex/release-3b-on-428`; draft PR #10 created against `main`; observed `OPEN` / `isDraft=true` / `mergeable=MERGEABLE`; no merge |
