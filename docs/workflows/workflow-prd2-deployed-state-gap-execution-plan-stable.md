...
| Production deploy identity | 2026-06-24 M5 Public Content scheduler approval production gate 后，remote `HEAD` 和 `.deploy-sha` 均为 `a81154426fd4e942fc9439de3dcbd9c816122562` | L3 deployment evidence；该 SHA 是当前生产代码点 |
...
| Public content retained cleanup local policy | 本地新增 `data_intelligence_hub.maintenance.public_content_retention` 与 `scripts/cleanup-retained-public-content.sh`，默认 dry-run，限定 `retained-public-content-*@example.com` 且只清理 public-content Source/Task/Run/Dataset/DriftEvent/Report/Export graph；执行模式会在删除前阻断 export artifact root 外路径 | L2 local validation；证明 cleanup scope、TTL cutoff、artifact-root safety check 和 protected fixture exclusion；不证明 production deploy、production cleanup dry-run、production cleanup execute 或 retained canary deletion |
...
| Release commit | `a81154426fd4e942fc9439de3dcbd9c816122562` 已发布到 `/opt/data-achieve-scrapy/app` | L4 M5 public content scheduler approval production gate 对应生产部署；后续本地测试/文档提交不代表生产已同步 |
| Source main state | 生产服务器当前 deployed commit 为本地 gate 分支经 bundle fast-forward 后的 `a81154426fd4e942fc9439de3dcbd9c816122562` | L1/L3 source and deployment evidence；origin/main 是否同步到该 SHA 仍需单独推送/PR 证据 |
...
4. 生产 HEAD `a81154426fd4e942fc9439de3dcbd9c816122562` 已包含 PRD2/M1/M2/M3/M5 的多项实现：平台包、CapabilityProbe、BrowserDiagnosticRun/Job/JobRun、browser local runner、GitHub Tool Radar 深化字段、schema/provenance、report risk sections、drift signal groups、独立站 dataset/export/drift/report、`public_feed`、`generic_web.v1`、`public_content_update.v1` Dataset/drift/report preview、`public_content` Report asset 持久化、`public_content_drift` DatasetDriftEvent save/list/reuse，以及 public-content schedule approval mutation；本地工作树另外新增 retained public-content cleanup policy tooling，尚未部署生产。
5. 生产 Alembic head 已到 `202606110023`，PRD2 R0 release/schema gap 已闭合；M3 GitHub API-first 小范围生产 package gate 已完成并清理；M5 public content 小范围生产 package smoke、Report asset gate、Dataset export gate 已完成并清理；M5 retained lifecycle gate 已保留一组 canary Dataset/Report/Export asset；public-content drift event 专用持久化、docs/page production gate 和 scheduler approval gate 均已完成 scoped gate 并清理；retained cleanup policy 已本地完成但未生产 dry-run/execute。剩余 gap 转为更大 scope rate-limit、生产 retained cleanup 验证、多日 TTL、scheduler tick/recurring monitoring、provider/email 等独立 gate。

### Inferences

1. 线上产品已经完成 PRD2 R0 release/schema 对齐，并完成一次小范围 M3 GitHub API-first L4 package gate、一次 M5 public content L4 package smoke、一次 M5 Report asset gate、一次 M5 Dataset export gate、一次 M5 retained lifecycle gate 与一次 M5 scheduler approval mutation gate；本地已补齐 retained cleanup policy tooling；这不等于大规模 recurring collection、scheduler tick、多日 TTL、production cleanup dry-run/execute 或自动清理生命周期也已完成。
...
2. retained lifecycle gate 只验证了即时保留、重登录后可见、DB/volume 中 canary asset 存在；本地 cleanup policy 已验证 TTL cutoff、scope 和 artifact-root safety；但没有验证 production cleanup dry-run/execute、多日保留、自动清理、定时刷新后的生命周期；retained canary 本身仍未被本轮 drift-event/docs-page/scheduler approval gate 更新。
3. 生产 GitHub package gate 覆盖 Source/Task write、一次 GitHub API TaskRun、Dataset save、report asset、drift snapshot；M5 scheduler approval gate 只覆盖 schedule metadata mutation，不覆盖 provider enrichment、邮件发送、scheduler tick execution、dataset export 或浏览器运行。
...
| Independent site | Shopify-style 商品发现、fan-out、dataset、drift、export | 本轮未执行 M4 授权测试站写入 E2E | `origin/main=e97810a` 基线已随当前 `a81154426fd4e942fc9439de3dcbd9c816122562` 生产代码点保留 | M4-4 授权测试站 E2E 未完成；需要测试站 URL、cleanup register 和 export/retention 边界 | P0/M4 |
| Public Web/RSS/Docs | 公开网页、RSS/Atom、docs 更新监控平台包 | M5 production smoke 已跑通 `public_feed` RSS TaskRun、`public_content_update` DatasetVersion、read-only drift/report preview；Report asset、Dataset export、retained lifecycle canary、scoped `public_content_drift` event gate、`generic_web` docs/page production gate 和 public-content scheduler approval gate 均已完成 | M5 local scaffold、Dataset/drift/report slice、Report asset/API/Web client/export contract、public-content drift event persistence、`generic_web` docs/page hash diff local slice、scheduler approval path、retained cleanup policy tooling 和测试已完成 | scheduler tick/recurring monitoring、production retained cleanup dry-run/execute、多日 TTL 仍未闭合；retained canary 未被 drift-event/docs-page/scheduler gate 更新 | Done/M5 |
...
当前最重要的 gap 已从“本地最新 PRD2/M2 能力和线上部署证据没有对齐”，转为“GitHub/M5 小范围 L4 gates 已完成，但更大 scope、长期保留、scheduler tick/recurring monitoring、provider/email 和下一平台包仍需分 gate 推进”。
...
| Remaining L4 breadth | GitHub 小范围 L4 已完成；M5 public-content 多个生产 gate 已完成；其他平台、邮件、provider、scheduler tick/recurring monitoring 仍未执行 | 继续按 single-step authorization envelope 执行，每次保留 cleanup 或 retention evidence |
...
R0-5 已完成；R0-6 已在 M3 GitHub 小范围 scope 下完成；M5 public content production smoke、Report asset gate、Dataset export gate、docs/page gate 和 scheduler approval gate 已完成。M4、provider/email、scheduler tick/recurring monitoring 和生产浏览器运行仍不能宣称生产写入完成，除非各自获得单独授权并留下 cleanup 或 retention evidence。
...
| M4-3 | Dataset/drift 样例 | dataset/drift tests、docs | done_main_and_deployed_in_current_baseline：新增/下架、价格变化可进入 DatasetDriftEvent；随 `origin/main=e97810a` 基线保留在当前 `a81154426fd4e942fc9439de3dcbd9c816122562` 生产代码点 |
...
|---|---|---|---|
| Done | Release boundary, migration to `023`, M3 GitHub package gate, M5 public content smoke, M5 Report asset gate, M5 Dataset export gate, M5 retained lifecycle gate, M5 public-content drift event production gate, M5 docs/page production gate, M5 public-content scheduler approval gate, M5 retained cleanup policy local slice | production HEAD `a811544`，schema `202606110023`，小范围 L4 GitHub package gate、M5 public content smoke、M5 Report asset gate、M5 Dataset export gate、M5 retained canary、public-content drift event save/reuse、docs/page `generic_web` gate、scheduler approval mutation、cleanup/retention evidence 和本地 retained cleanup policy 完成 | release/evidence |
...
| P1 | Public Web/RSS/Docs next gates | M5 base production smoke、Report asset gate、Dataset export gate、retained lifecycle gate、production drift-event gate、docs/page hash diff local slice、production docs/page gate、scheduler approval gate 和本地 retained cleanup policy 已完成；下一步只剩 production retained cleanup dry-run/execute decision、scheduler tick/recurring monitoring、retained canary refresh 等独立 gate | URL/feed/docs collector |
...
1. M5 public content production package smoke、Report asset gate、Dataset export gate、retained lifecycle gate、production drift-event gate、docs/page hash diff local slice、production docs/page gate、scheduler approval gate 和 retained TTL/cleanup policy local slice 已完成；下一步可选择一个独立授权 gate：部署 cleanup tooling 后先跑 production retained cleanup dry-run、scheduler tick/recurring monitoring、retained canary refresh 或更大 scope rate-limit gate 之一。
2. 如继续 M4-4，需要明确测试站 URL、允许写入资源、cleanup register、是否允许 export file，以及 cleanup dry-run/execute。
3. M5 已完成小范围生产写入、Report asset 创建、Dataset export 创建/下载/删除、retained canary 保留、public-content drift event 创建/复用、docs/page `generic_web` gate、scheduler approval mutation、本地 retained cleanup policy 和清理边界验证；仍不做 provider call、email、scheduler tick/recurring monitoring、production retained cleanup execute 或 browser run，除非另起授权 gate。
