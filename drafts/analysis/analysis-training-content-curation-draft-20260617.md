...
- source snapshot: `tmp/outputs/training-content-snapshot-20260617.json`
- raw records: 69
- entities: 69
- signals: 13
- intelligence items: 20
...
| `intel-agent-browser-understanding` | Agent 浏览器训练要把页面理解、网络观察和人工复核连成闭环 | `ai_agent_collection` | 4 | 0.832 |
| `intel-ai-extraction-pipeline-diversifies` | AI 抽取工具已从 crawler 扩展到语义查询、视觉流程和 graph pipeline | `ai_agent_collection` | 3 | 0.796 |
| `intel-rpa-no-code-collection-track` | RPA 和 no-code 采集应成为业务培训主线，而不是开发者工具的附录 | `rpa_automation` | 7 | 0.803 |
| `intel-open-source-no-code-bridges-training` | 开源 no-code 采集平台能连接业务培训和工程化采集 | `no_code_scraping` | 3 | 0.761 |
| `intel-browser-preflight-before-scraping` | 浏览器采集前必须先做公开暴露面和授权范围预检 | `osint_preflight` | 5 | 0.835 |
| `intel-fingerprint-risk-is-training-boundary` | 浏览器指纹和反检测工具只能作为风险教育，不应进入绕过式 SOP | `browser_risk` | 3 | 0.844 |
