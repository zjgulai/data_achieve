...
| Production deploy identity | 2026-06-24 M5 Public Content docs/page production gate 后，remote `HEAD` 和 `.deploy-sha` 均为 `af23cefc92aa9fec336f632a5b1561623811c2fd` | L3 deployment evidence；该 SHA 是当前生产代码点 |
...
| Release commit | `af23cefc92aa9fec336f632a5b1561623811c2fd` 已发布到 `/opt/data-achieve-scrapy/app` | L4 M5 public content docs/page production gate 对应生产部署；后续本地测试/文档提交不代表生产已同步 |
| Source main state | 生产服务器 `main...origin/main [ahead 16]`，当前 deployed commit 为本地 gate 分支经 bundle fast-forward 后的 `af23cefc92aa9fec336f632a5b1561623811c2fd` | L1/L3 source and deployment evidence；origin/main 是否同步到该 SHA 仍需单独推送/PR 证据 |
...
| Independent site | Shopify-style 商品发现、fan-out、dataset、drift、export | 本轮未执行 M4 授权测试站写入 E2E | `origin/main=e97810a` 基线已随当前 `af23cefc92aa9fec336f632a5b1561623811c2fd` 生产代码点保留 | M4-4 授权测试站 E2E 未完成；需要测试站 URL、cleanup register 和 export/retention 边界 | P0/M4 |
| Public Web/RSS/Docs | 公开网页、RSS/Atom、docs 更新监控平台包 | M5 production smoke 已跑通 `public_feed` RSS TaskRun、`public_content_update` DatasetVersion、read-only drift/report preview；Report asset、Dataset export、retained lifecycle canary、scoped `public_content_drift` event gate 和 `generic_web` docs/page production gate 均已完成 | M5 local scaffold、Dataset/drift/report slice、Report asset/API/Web client/export contract、public-content drift event persistence、`generic_web` docs/page hash diff local slice 和测试已完成 | recurring monitoring、scheduler、多日 TTL/自动清理仍未闭合；retained canary 未被 drift-event/docs-page gate 更新 | Done/M5 |
...
| M4-3 | Dataset/drift 样例 | dataset/drift tests、docs | done_main_and_deployed_in_current_baseline：新增/下架、价格变化可进入 DatasetDriftEvent；随 `origin/main=e97810a` 基线保留在当前 `af23cefc92aa9fec336f632a5b1561623811c2fd` 生产代码点 |
...
|---|---|---|---|
| Done | Release boundary, migration to `023`, M3 GitHub package gate, M5 public content smoke, M5 Report asset gate, M5 Dataset export gate, M5 retained lifecycle gate, M5 public-content drift event production gate, M5 docs/page production gate | production HEAD `af23cef`，schema `202606110023`，小范围 L4 GitHub package gate、M5 public content smoke、M5 Report asset gate、M5 Dataset export gate、M5 retained canary、public-content drift event save/reuse、docs/page `generic_web` gate 和 cleanup/retention evidence 完成 | release/evidence |
...
1. M5 public content production package smoke、Report asset gate、Dataset export gate、retained lifecycle gate、production drift-event gate、docs/page hash diff local slice 和 production docs/page gate 已完成；下一步可选择一个独立授权 gate：scheduler approval、多日 TTL/cleanup policy、retained canary refresh 或更大 scope rate-limit gate 之一。
2. 如继续 M4-4，需要明确测试站 URL、允许写入资源、cleanup register、是否允许 export file，以及 cleanup dry-run/execute。
3. M5 已完成小范围生产写入、Report asset 创建、Dataset export 创建/下载/删除、retained canary 保留、public-content drift event 创建/复用、docs/page `generic_web` gate 和清理边界验证；仍不做 provider call、email、scheduler 或 browser run，除非另起授权 gate。
