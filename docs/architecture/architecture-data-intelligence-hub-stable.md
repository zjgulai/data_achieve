...
6. 截至 commit `20c4bd2`，Automation 平台包、采集计划、清洗计划、数据集保存、GitHub Topic Radar、公开网页结构预检、工具雷达 Report 资产和生产浏览器链路已完成一轮生产验收；后续重点是浏览器结构解析预检、更多平台包真实采集深度与长期运行可靠性。
...
截至 2026-06-19，commit `20c4bd2` 已验证：
...
5. 生产运行目录为 `/opt/data-achieve-scrapy/app`，该目录当前 HEAD 为 `20c4bd2`；远程 GitHub fetch 曾遇到传输失败，本次部署采用本地已推送 commit 的 git bundle fast-forward。
