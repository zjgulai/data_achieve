...
2. retained lifecycle gate 只验证了即时保留、重登录后可见、DB/volume 中 canary asset 存在；没有验证多日保留、TTL、自动清理、定时刷新后的生命周期；retained canary 本身仍未被本轮 drift-event/docs-page/scheduler approval gate 更新。
...
|---|---|---|---|
| Done | Release boundary, migration to `023`, M3 GitHub package gate, M5 public content smoke, M5 Report asset gate, M5 Dataset export gate, M5 retained lifecycle gate, M5 public-content drift event production gate, M5 docs/page production gate, M5 public-content scheduler approval gate | production HEAD `a811544`，schema `202606110023`，小范围 L4 GitHub package gate、M5 public content smoke、M5 Report asset gate、M5 Dataset export gate、M5 retained canary、public-content drift event save/reuse、docs/page `generic_web` gate、scheduler approval mutation 和 cleanup/retention evidence 完成 | release/evidence |
...
1. M5 public content production package smoke、Report asset gate、Dataset export gate、retained lifecycle gate、production drift-event gate、docs/page hash diff local slice、production docs/page gate 和 scheduler approval gate 已完成；下一步可选择一个独立授权 gate：scheduler tick/recurring monitoring、多日 TTL/cleanup policy、retained canary refresh 或更大 scope rate-limit gate 之一。
2. 如继续 M4-4，需要明确测试站 URL、允许写入资源、cleanup register、是否允许 export file，以及 cleanup dry-run/execute。
3. M5 已完成小范围生产写入、Report asset 创建、Dataset export 创建/下载/删除、retained canary 保留、public-content drift event 创建/复用、docs/page `generic_web` gate、scheduler approval mutation 和清理边界验证；仍不做 provider call、email、scheduler tick/recurring monitoring 或 browser run，除非另起授权 gate。
