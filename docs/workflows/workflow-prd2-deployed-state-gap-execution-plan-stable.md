...
2026-06-29 当前生产只读核验显示：`GET https://scrapy.lute-tlz-dddd.top/api/health` 仍返回 `environment=production`、`status=ok`、`database=connected`、`schema=current`、`schema_revision=202606110023`、`schema_head=202606110023`、`scheduler_enabled=true`；active app working tree `/opt/data-achieve-scrapy/app` 的 `HEAD` 与 `/opt/data-achieve-scrapy/app/.deploy-sha` 均为 `42851929d59d82708c9380d36347ca721979297d`；API/Web compose working directory 均为 `/opt/data-achieve-scrapy/app/configs/deploy/scrapy`；本地 main worktree 与 `origin/main` 同为 `42851929d59d82708c9380d36347ca721979297d`。本次核验为 L3 production read-only，不执行 production write、provider call、email send、cleanup execute、scheduler mutation、production browser run 或 browser artifact write。
...
| Production deploy identity | 2026-06-29 read-only probe: active app `HEAD=42851929d59d82708c9380d36347ca721979297d` and `/opt/data-achieve-scrapy/app/.deploy-sha=42851929d59d82708c9380d36347ca721979297d`; running API/Web compose working directory is `/opt/data-achieve-scrapy/app/configs/deploy/scrapy`; parent marker/current symlink remain separate housekeeping evidence, not current compose identity | L3 deployment evidence；当前 compose 部署身份应读取 app marker，不应读取父级旧 marker |
...
| Release commit | `42851929d59d82708c9380d36347ca721979297d` 已发布到 `/opt/data-achieve-scrapy/app`，active app marker 与 HEAD 匹配 | L3 deployment identity + health evidence；不代表本轮新文档/后续本地修改已发布 |
| Source main state | 本地 main worktree 与 `origin/main` 均为 `42851929d59d82708c9380d36347ca721979297d` | L1 source evidence；当前 feature branch 的文档修改尚未合并到 main |
...
4. 生产远端 app working tree `HEAD=42851929d59d82708c9380d36347ca721979297d` 已包含 PRD2/M1/M2/M3/M5 的多项实现：平台包、CapabilityProbe、BrowserDiagnosticRun/Job/JobRun、browser local runner、GitHub Tool Radar 深化字段、schema/provenance、report risk sections、drift signal groups、独立站 dataset/export/drift/report、`public_feed`、`generic_web.v1`、`public_content_update.v1` Dataset/drift/report preview、`public_content` Report asset 持久化、`public_content_drift` DatasetDriftEvent save/list/reuse、public-content schedule approval mutation，以及 retained public-content cleanup policy tooling 的 scheduler refresh run lineage fix；active app `.deploy-sha` 同样为 `42851929d59d82708c9380d36347ca721979297d`。
...
|---|---|---|---|---|---|
| FP-00 | PRD/architecture/gap state sync | 2026-06-29 L3 production read-only identity and health refreshed | Keep PRD2, architecture, gap plan aligned with `42851929` and known unfinished gates | P0 | Docs-only; no production mutation |
