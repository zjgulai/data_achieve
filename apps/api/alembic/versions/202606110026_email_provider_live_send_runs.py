"""Create email provider live send run table.

Revision ID: 202606110026
Revises: 202606110025
...
revision: str = "202606110026"
down_revision: str | None = "202606110025"
...
def upgrade() -> None:
    op.create_table(
        "email_provider_live_send_runs",
        sa.Column("workspace_id", sa.Uuid(), nullable=False),
        sa.Column("user_id", sa.Uuid(), nullable=False),
        sa.Column("gate_run_id", sa.Uuid(), nullable=False),
        ...
        sa.Column("live_approval_required", sa.Boolean(), nullable=False),
        sa.Column("recipient_allowlisted", sa.Boolean(), nullable=False),
        ...
        sa.ForeignKeyConstraint(["gate_run_id"], ["email_provider_live_gate_runs.id"]),
        sa.ForeignKeyConstraint(["user_id"], ["users.id"]),
        sa.ForeignKeyConstraint(["workspace_id"], ["workspaces.id"]),
        sa.PrimaryKeyConstraint("id"),
    )
    op.create_index(
        "ix_email_provider_live_send_runs_workspace_created",
        "email_provider_live_send_runs",
        ["workspace_id", "created_at"],
    )
    op.create_index(
        "ix_email_provider_live_send_runs_user_created",
        "email_provider_live_send_runs",
        ["user_id", "created_at"],
    )
    op.create_index(
        "ix_email_provider_live_send_runs_gate_created",
        "email_provider_live_send_runs",
        ["gate_run_id", "created_at"],
    )
    op.create_index(
        "ix_email_provider_live_send_runs_idempotency",
        "email_provider_live_send_runs",
...
def downgrade() -> None:
    op.drop_index(
        "ix_email_provider_live_send_runs_idempotency",
        table_name="email_provider_live_send_runs",
    )
    op.drop_index(
        "ix_email_provider_live_send_runs_gate_created",
        table_name="email_provider_live_send_runs",
    ...
    op.drop_table("email_provider_live_send_runs")
